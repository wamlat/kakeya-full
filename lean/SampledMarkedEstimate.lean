...
namespace KakeyaFormal.SampledMarkedEstimate
...
theorem construct {k : ℕ} {m d d' pExp qExp : ℝ}
  ...
  have hwidth : (1:ℝ)/12 ≤ width+((k+2:ℕ):ℝ)/2 := by
    ...
    push_cast
    linarith
  obtain ⟨cCore,hcCore,hcore⟩ := MarkedPivotEstimate.all_scales hbase hlift hm hpExp hd hqExp
...
end KakeyaFormal.SampledMarkedEstimate
