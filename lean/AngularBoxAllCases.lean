...
namespace KakeyaFormal.AngularBoxAllCases
...
theorem construct {k : ℕ} {m d d' pExp qExp : ℝ}
  ...
  have hmn' : m+1 ≤ ((k+1:ℕ):ℝ)+1 := by
    ...
    linarith
  obtain ⟨δh,hδh,_hδh1,cH,hcH,hHigh⟩ := AngularBoxHighDensity.construct hbase hlift
...
end KakeyaFormal.AngularBoxAllCases
