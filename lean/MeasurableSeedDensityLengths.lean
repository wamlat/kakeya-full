...
namespace KakeyaFormal.MeasurableSeedDensityLengths
open MeasureTheory Set DensityLogNormalization SamplingGeometry
open scoped ENNReal
noncomputable section
open Classical

/-- Source (4.4) with independent logarithmic lower and upper density losses,
arbitrary fixed physical width, finite variable axis-length upper bound, and positive direction-separation constants. No actual
...
theorem logarithmic_density_seed (k : ℕ)
  ...
  obtain ⟨c,P,hc,hP,hmain⟩ := MeasurableSeedLengths.natural_logarithmic_seed k
    (width:=width) hLength hseparation halpha hBF ht hm
  ...
  have henclose (i) : Y i ⊆ (F.tube i).carrier (upperLength+width*δ) := by
    intro x hx
    have hd := MeasurableSeedLengths.lengthCarrier_subset_closedBall (F.tube i) (length i) (width*δ) (hsub i hx)
    refine ⟨0,by norm_num,?_⟩
    have hb := Metric.mem_closedBall.mp hd
    simpa only [UnitTube.axisPoint,zero_smul,add_zero] using
      hb.trans (by linarith [hlength i])
  obtain ⟨O⟩ := MeasurableDensityTrimming.construct F Y hY henclose ha.le halower
  let BN := max 1 (D*L^t)
  ...
  have hnewEnds : ∀ i x r, δ ≤ r → r ≤ 1 →
    ...
    have hh := O.two_ends_upper ha hBpos henclose hupper i x (hδ.trans_le hr).le (hends i x r hr hr1)
    ...
    exact hh.trans hm'
  have hbound := hmain M F O.shading length δ lam BN A hδ hδ1 hlam.1 hlam.2 hBN hA hBNbudget
    hlength O.measurable (fun i => (O.subset_original i).trans (hsub i)) hlowerNew hupperNew hsep hcap hnewEnds
  have hunion := div_le_div_of_nonneg_right (O.union_volume_le henclose) (pow_pos hδ (k+2)).le
...
end KakeyaFormal.MeasurableSeedDensityLengths
