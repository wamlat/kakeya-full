import PositivePivotSlabs
import PositivePivotAlgebra
import Endpoint
...
scale restriction is the explicit original N*kappa^20 cutoff. The full source
range p>0 is covered by actual per-tube trimming, without Jensen. -/
namespace KakeyaFormal.SourceMarkedPivotFullRange
...
theorem admissible_fourth {k : ℕ} {m d d' p q : ℝ}
  ...
  obtain ⟨K,hK,hpackage⟩ := PositivePivotSlabs.construct
  ...
  exact PositivePivotAlgebra.fourth_power U hK hBase hClose
...
theorem source_fourth {k : ℕ} {m d d' p q : ℝ}
  ...
  exact PositivePivotAlgebra.natural_log_fourth h.scale_pos h.scale_le_one hc
...
Every positive base-density power p is allowed. -/
...
end KakeyaFormal.SourceMarkedPivotFullRange
